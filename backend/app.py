...
def load_my_responses():
    try:
        ...
        st.error("Error: 'my_response.json' file not found. Personal chatbot features disabled.")
        return {}
    except json.JSONDecodeError:
        st.error("Error: 'my_response.json' has a formatting error. Personal chatbot features disabled.")
        return {}
...
if submitted and user_q:
    ...
    if len(chunks) == 0:
        ...
        out = "I'm here to listen. (That trigger isn't defined.) Please upload a PDF or .txt to ask about a document."

        for trigger in my_responses.keys():
            if trigger.lower() in user_input:
                out = my_responses[trigger]
                response_found = True
                break
    else:
